function [comap,A1,world]=definewoco2(filename,typ,path)
...
% disp('Please indicate the region for calibration')
...
inp=1;
  ...
  while inp~=0,
      ...
      [maske(tel).msk,maske(tel).idx,maske(tel).idy]=roipoly(b1);  
      hold on
  ...
  b2=zeros(size(b1)); b3=b2;
  for itel=1:tel-1
      b2=b1.*maske(itel).msk;
      b3=b2+b3;
  end
  figure, imshow(b3);
