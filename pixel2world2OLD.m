...
%expression for conversion corrected 22-08-2013 R. Aleixo
...
elseif strcmp(mapping,'nonlinear')==1
  if length(lswo1)<4  || lswo1(3)>lswo2(1) %lswo(3)>lswo(1)
...
for ii=1:1:size(upix,2)
  for jj=1:1:size(upix,1)
    ...
    x(jj,ii)=lswo1(1)+ lswo1(2)*xpix(jj,ii)+ lswo1(3)*ypix(jj,ii)+...
	  lswo1(6)*(xpix(jj,ii).*ypix(jj,ii))+...
	  lswo1(4)*(xpix(jj,ii).^2)+lswo1(5)*(ypix(jj,ii).^2);

  y(jj,ii)=lswo2(1)+ lswo2(2)*xpix(jj,ii)+ lswo2(3)*ypix(jj,ii)+...
	  lswo2(6)*(xpix(jj,ii).*ypix(jj,ii))+...
	  lswo2(4)*(xpix(jj,ii).^2)+lswo2(5)*(ypix(jj,ii).^2);
  end
end
