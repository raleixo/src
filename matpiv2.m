...
elseif nargin==7
  [im1,im2,winsize,Dt,overlap,method,msk]=deal(varargin{:});
   ustart=[];vstart=[]; %ms='';
  if (ischar(msk) && ~isempty(msk)), mss=load(msk);ms=mss.maske; else ms=msk; end
   ustart=[];vstart=[];
   
elseif nargin==8
    [im1,im2,winsize,Dt,overlap,method,msk,wocofile]=deal(varargin{:});
    if (ischar(msk) && ~isempty(msk)), mss=load(msk);ms=mss.maske; else ms=msk; end
    ustart=[];vstart=[];
...
if nargin>7 || nargin==1
    if ~isempty(wocofile)
        ...
        if size(D,1)==1
            mapp=load(D(1).name);
            [xw,yw,uw,vw]=pixel2world2(u,v,x,y,mapp.comap(:,1),mapp.comap(:,2),'nonlinear');
        else
