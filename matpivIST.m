function [xw,yw,uw,vw,SnR,Pkh,u2,v2]=matpivIST(varargin)
% TimeStamp: November2014
% MATPIV - the Particle Image Velocimetry toolbox for MATLAB
%
% MATPIVIST: changed for PIV processing with multipassxNOFILTER

%
% function [xw,yw,uw,vw,snr,pkh]=matpiv(im1,im2,winsize,Dt,overlap...
% ,method,SubPixMethod,wocofile,mask,ustart,vstart)
%
% SubPixMethod extended included by Ana Margarida
%
...
% SubPixMethod efines the method for subpixel interpolation (1 = centroid fit; 
% 2 = gaussian fit; 3 = parabolic fit)
%
...
if nargin==5
  [im1,im2,winsize,Dt,overlap]=deal(varargin{:}); ms=''; ustart=[];vstart=[]; SubPixMethod=2;
elseif nargin==6
  [im1,im2,winsize,Dt,overlap,method]=deal(varargin{:}); SubPixMethod=2; ms='';
   ustart=[];vstart=[];
elseif nargin==7
    [im1,im2,winsize,Dt,overlap,method,SubPixMethod]=deal(varargin{:}); ms='';
    ustart=[];vstart=[];
elseif nargin==8
  [im1,im2,winsize,Dt,overlap,method,SubPixMethod,wocofile]=deal(varargin{:});
  ms=''; ustart=[];vstart=[]; 
elseif nargin==9
  [im1,im2,winsize,Dt,overlap,method,SubPixMethod,wocofile,msk]=deal(varargin{:});
  if (ischar(msk) && ~isempty(msk)), mss=load(msk);ms=mss.maske; else, ms=msk; end
   ustart=[];vstart=[];
elseif nargin==11
  [im1,im2,winsize,Dt,overlap,method,SubPixMethod,wocofile,msk,ustart,vstart]=deal(varargin{:});
  if ischar(msk), mss=load(msk);ms=mss.maske; end
end

if strcmp(method,'single')==1
  [x,y,u,v,SnR,Pkh,u2]=singlepass(im1,im2,winsize,Dt,overlap,SubPixMethod,ms);
...
elseif ~isempty(findstr(method,'multi')) %~=0
    ...
    if strcmp(method,'multin') || size(winsize,1)>1
      [x,y,u,v,SnR,Pkh]=multipassxNOFILTER(im1,im2,winsize,...
				   Dt,overlap,SubPixMethod,validvec,ms,iter, ...
				   ustart,vstart); 
...
elseif (nargin<=8 && nargin>1) || isempty(wocofile)
