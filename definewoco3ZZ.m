%     seqcalib.status=0;    %1 if calibration already made
%     
%     seqcalib.path=makecase.calibration;
%     seqcalib.name='Cam01calib02_0001.bmp';
%     seqcalib.calibration=makecase.calibration; %folder where calibration image is stocked
%     seqcalib.output=makecase.calibration; %folder where calibration data will be stocked
%     
%    % if ~seqcalib.status
%         B=imread([seqcalib.path '\' seqcalib.name]);
%         if size(B,3)>1
%             B=rgb2gray(B);
%         end
%  %       definewoco3(B,'s',seqcalib.output);  %calibration part
%   %  end



function [comap,A1,world]=definewoco3ZZ(filename,typ,path)
...
format long
%ilename=[seqcalib.path seqcalib.name];
% typ='.';
% A=B;
if ischar(filename)
  A=imread(filename);
...
while strcmp(inpx,'inne')
  ...
  if strcmp(mapfun,'N')==1 || strcmp(mapfun,'n')==1
    if length(world)>=6
       A1=[ones(size(x,1),1) x y (x.^2) (y.^2) (x.*y)];%A1=[ones(size(x,1),1) x.' y.' (x.*y).' (x.^2).' (y.^2).'];
      inpx='ute';
    else
